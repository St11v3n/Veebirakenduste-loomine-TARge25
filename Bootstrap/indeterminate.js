    //ühendab html-s oleva id parent const-iks ja kõik class-iga 
    //child-check olevad elemendid children-iks
    ...
    //ja vastavalt sellele muddab parent-checki checked ja indeterminate omadusi
    function updateParent() {
        //loeme, mitu child-check on checked
        const checked = [...children].filter(c => c.checked).length;
        //=== võrdub väärtusega ja tüübiga, 
        //== kas kaks muutujat on väärtuselt võrdsed, kui tüüp võib olla erinev
        //= see on lihtsalt omistamine, kui ühele poolele pannakse väärtus
        // ja seejärel see väärtus omistatakse teisele poolele. Nt, let x = 5;
        if (checked === 0) {
            parent.checked = false;
            parent.indeteterminate = false;
    ...
    //mis kutsuvad updateParent funktsioni
    children.forEach(c => c.addEventListener("change", updateParent));
    
    //lisa event listener parent_check elementile,
